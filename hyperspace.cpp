...
#include <stdio.h>
 ...
 const map< string, HyperCube >* HyperSpace::determineSpatialRegions(){
     ...
     unsigned *num_partitions = new unsigned[this->dimension]; // Number of partitions of each spatial component
     ...
     double *initial_upp_bounds = new double[this->dimension];
     for(unsigned i=0 ; i < this->dimension ; i++){
     ...
     this->createHyperCubes( num_partitions , initial_upp_bounds_str, this->hypercubeEdgeLenght() ); // this line the error occurs.
 
	 delete [] initial_upp_bounds;
 ...
 void HyperSpace::createHyperCubes( const unsigned *num_partitions , const string& upp_bounds_str  , double edge_length){
     ...
     double *low_bounds = new double[this->dimension];
     for(unsigned i=0 ; i < this->dimension ; i++){
     ...
     /* Set first neighbor */
     int *neighbor = new int[this->dimension];
     for(unsigned i=0 ; i < this->dimension; i++){
     ...
     for(unsigned i=0 ; i < num_neighbors; i++){
         ...
         double *next_upp_bounds = new double[this->dimension];
         double *next_low_bounds = new double[this->dimension];
		 ...
		 delete [] next_upp_bounds;
		 delete [] next_low_bounds;
     ...
     for( ; it != neighbors_keys.end() ; it++){
         ...
         if( this->hypercubes.count(*it) > 0 )  continue;
        	 ...
        	 //int k = this->hypercubes.size();
        	 //map<string,HyperCube>::iterator begin,end;
        	 //begin = this->hypercubes.begin();
        	 //int fc = 0;
        	 //for(;begin != this->hypercubes.end();begin++)
        	 //{
        	//	 printf("%d %d %s\n",fc,k,(*begin).first.c_str());
        		// fc++;
        	 //}
 ...
 void HyperSpace::insertEntity( const DatasetEntity& entity ){
     ...
     double *attr_values = new double[this->dimension];
     for(unsigned i=0 ; i < this->dimension ; i++){
 
         double sigma = this->hypercubeEdgeLenght();
         attr_values[i] = sigma * (floor( (entity.getComponentValue(i) - this->lower_bounds[i]) / sigma) + 1) + this->lower_bounds[i];
